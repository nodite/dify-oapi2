...
class Chunk:
    def list(self, request, option) -> ListChildChunksResponse: ...
    def create(self, request, option) -> CreateChildChunkResponse: ...
    def update(self, request, option) -> UpdateChildChunkResponse: ...
    def delete(self, request, option) -> DeleteChildChunkResponse: ...
    # + async versions
```
...
- Remove `list`, `create`, `update`, `delete` from Segment class
