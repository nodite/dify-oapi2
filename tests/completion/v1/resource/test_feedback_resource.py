...
class TestFeedbackResource:
    ...
    def message_feedback_request(self) -> MessageFeedbackRequest:
        request_body = (
            MessageFeedbackRequestBody.builder().rating("like").user("test-user").content("Great response!").build()
        )
        return MessageFeedbackRequest.builder().message_id("test-message-id").request_body(request_body).build()

    @pytest.fixture
    def get_feedbacks_request(self) -> GetFeedbacksRequest:
        return GetFeedbacksRequest.builder().page("1").limit("20").build()
