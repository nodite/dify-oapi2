...
from dify_oapi.api.knowledge_base.v1.model.dataset.retrieve_response import RetrieveDatasetResponse, QueryInfo
...
class TestComprehensiveIntegration:
    ...
    def test_complete_knowledge_base_setup_workflow(
        ...
        responses = [
            # 1. Create dataset
            CreateDatasetResponse(id=dataset_id, name="Comprehensive Dataset", description="Full workflow test"),
            ...
            ListMetadataResponse(
                doc_metadata=[MetadataInfo(id=metadata_id, name="Document Category", type="string", use_count=0)],
                built_in_field_enabled=True
            ),
            
            # 4. Create tag
            CreateTagResponse(id=tag_id, name="Technical Documentation", type="knowledge", binding_count=0),
            ...
            QueryBoundTagsResponse(
                data=[TagInfo(id=tag_id, name="Technical Documentation")],
                total=1
            ),
            
            # 7. Retrieve from dataset with metadata filtering
            RetrieveDatasetResponse(
                query=QueryInfo(content="comprehensive test query"),
                records=[]
            )
        ]
        ...
        # 1. Create dataset
        create_dataset_request = CreateDatasetRequest.builder().name("Comprehensive Dataset").description("Full workflow test").build()
        ...
        create_metadata_request = CreateMetadataRequest.builder().dataset_id(dataset_id).type("string").name("Document Category").build()
        ...
        create_tag_request = CreateTagRequest.builder().name("Technical Documentation").build()
        ...
        # 5. Bind tag to dataset
        bind_tag_request = BindTagsRequest.builder().tag_ids([tag_id]).target_id(dataset_id).build()
        ...
        retrieve_request = RetrieveDatasetRequest.builder().dataset_id(dataset_id).query("comprehensive test query").build()
    ...
    async def test_complete_workflow_async(
        ...
        create_dataset_request = CreateDatasetRequest.builder().name("Async Comprehensive Dataset").build()
        created_dataset = await dataset_resource.acreate(create_dataset_request, request_option)
        assert created_dataset.id == dataset_id
        
        create_metadata_request = CreateMetadataRequest.builder().dataset_id(dataset_id).type("number").name("Priority Level").build()
    ...
    def test_cross_resource_dependencies(
        ...
        create_metadata_request = CreateMetadataRequest.builder().dataset_id(dataset_id).type("string").name("Test Field").build()
        ...
        bind_request = BindTagsRequest.builder().tag_ids(["existing-tag"]).target_id(dataset_id).build()
    ...
    def test_multiple_datasets_with_shared_resources(
        ...
        responses = [
            ...
            # Create shared tag
            CreateTagResponse(id=shared_tag_id, name="Shared Category", type="knowledge", binding_count=0),
        ...
        # Create two datasets
        create_request1 = CreateDatasetRequest.builder().name("Shared Dataset 1").build()
        dataset1 = dataset_resource.create(create_request1, request_option)
        assert dataset1.id == dataset1_id
        
        create_request2 = CreateDatasetRequest.builder().name("Shared Dataset 2").build()
        ...
        bind_request1 = BindTagsRequest.builder().tag_ids([shared_tag_id]).target_id(dataset1_id).build()
        bind_result1 = tag_resource.bind_tags(bind_request1, request_option)
        assert bind_result1.result == "success"
        
        bind_request2 = BindTagsRequest.builder().tag_ids([shared_tag_id]).target_id(dataset2_id).build()
    ...
    def test_advanced_retrieval_with_metadata_filtering(
        ...
        filter_condition = FilterCondition.builder().name("category").comparison_operator("is").value("technical").build()
        filtering_conditions = MetadataFilteringConditions.builder().logical_operator("and").conditions([filter_condition]).build()
        
        # Create retrieval model with filtering
        retrieval_model = RetrievalModel.builder().search_method("semantic_search").top_k(5).metadata_filtering_conditions(filtering_conditions).build()
        
        # Mock filtered retrieval response
        retrieve_response = RetrieveDatasetResponse(
            query=QueryInfo(content="filtered query"),
            records=[]
        )
        ...
        retrieve_request = RetrieveDatasetRequest.builder().dataset_id(dataset_id).query("filtered query").retrieval_model(retrieval_model).build()
    ...
    def test_error_propagation_across_resources(
        ...
        create_dataset_request = CreateDatasetRequest.builder().name("Error Dataset").build()
        ...
        create_metadata_request = CreateMetadataRequest.builder().dataset_id("error-dataset").type("string").name("Error Field").build()
    ...
    def test_resource_cleanup_workflow(
        ...
        # 2. Delete metadata
        delete_metadata_request = DeleteMetadataRequest.builder().dataset_id(dataset_id).metadata_id(metadata_id).build()
