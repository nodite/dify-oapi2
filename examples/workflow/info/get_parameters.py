...
def get_parameters_sync() -> None:
    try:
        ...
        response = client.workflow.v1.info.get_parameters(req, req_option)

        if response.success:
            print(f"Parameters: {len(response.user_input_form or [])} input forms")
        else:
...
async def get_parameters_async() -> None:
    try:
        ...
        response = await client.workflow.v1.info.aget_parameters(req, req_option)
