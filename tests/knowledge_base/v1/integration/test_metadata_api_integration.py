...
class TestMetadataAPIIntegration:
    ...
    def test_metadata_management_workflow_sync(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
        ...
        # 1. Create metadata
        create_request = CreateMetadataRequest.builder().dataset_id(dataset_id).type("string").name("Test Metadata").build()
        ...
        # 3. Update metadata
        update_request = UpdateMetadataRequest.builder().dataset_id(dataset_id).metadata_id(metadata_id).name("Updated Metadata").build()
    ...
    @pytest.mark.asyncio
    async def test_metadata_management_workflow_async(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
        ...
        list_response = ListMetadataResponse(doc_metadata=[MetadataInfo(id=metadata_id, name="Async Metadata", type="number", use_count=0)], built_in_field_enabled=False)
        update_response = UpdateMetadataResponse(id=metadata_id, type="number", name="Updated Async Metadata")
        delete_response = DeleteMetadataResponse()
        
        mock_aexecute = AsyncMock()
        mock_aexecute.side_effect = [create_response, list_response, update_response, delete_response]
        monkeypatch.setattr("dify_oapi.core.http.transport.ATransport.aexecute", mock_aexecute)
        
        # Execute async workflow
        create_request = CreateMetadataRequest.builder().dataset_id(dataset_id).type("number").name("Async Metadata").build()
        ...
        update_request = UpdateMetadataRequest.builder().dataset_id(dataset_id).metadata_id(metadata_id).name("Updated Async Metadata").build()
    ...
    def test_builtin_metadata_operations(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
    ...
    @pytest.mark.asyncio
    async def test_builtin_metadata_operations_async(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
    ...
    def test_document_metadata_update(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
        """Test document metadata update operations"""
        dataset_id = "doc-metadata-dataset"
        
        # Mock document metadata update
        from dify_oapi.api.knowledge_base.v1.model.metadata.update_document_request import OperationData, DocumentMetadata
        ...
        update_request = UpdateDocumentMetadataRequest.builder().dataset_id(dataset_id).operation_data([operation_data]).build()
        result = metadata_resource.update_document(update_request, request_option)
        assert result.result == "success"

    @pytest.mark.asyncio
    async def test_document_metadata_update_async(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
    ...
    def test_metadata_types_and_validation(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
        ...
        for metadata_type in metadata_types:
            ...
            create_request = CreateMetadataRequest.builder().dataset_id(dataset_id).type(metadata_type).name(f"Test {metadata_type.title()} Field").build()
    ...
    def test_error_scenarios(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
        """Test error handling scenarios"""
        # Mock error response
        mock_execute = Mock(side_effect=Exception("Metadata API Error"))
        monkeypatch.setattr("dify_oapi.core.http.transport.Transport.execute", mock_execute)
        
        create_request = CreateMetadataRequest.builder().dataset_id("error-dataset").type("string").name("Error Field").build()
    ...
    def test_edge_cases(self, metadata_resource: Metadata, request_option: RequestOption, monkeypatch: Any) -> None:
