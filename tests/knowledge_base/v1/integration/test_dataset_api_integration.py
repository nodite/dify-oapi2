...
class TestDatasetAPIIntegration:
    ...
    def test_dataset_lifecycle_sync(self, dataset_resource: Dataset, request_option: RequestOption, monkeypatch: Any) -> None:
        ...
        mock_create = Mock(return_value=create_response)
        ...
        mock_get = Mock(return_value=get_response)
        ...
        mock_update = Mock(return_value=update_response)
        ...
        mock_retrieve = Mock(return_value=retrieve_response)
        
        # Mock delete dataset
        delete_response = DeleteDatasetResponse()
        mock_delete = Mock(return_value=delete_response)
    ...
    @pytest.mark.asyncio
    async def test_dataset_lifecycle_async(self, dataset_resource: Dataset, request_option: RequestOption, monkeypatch: Any) -> None:
    ...
    def test_dataset_list_with_pagination(self, dataset_resource: Dataset, request_option: RequestOption, monkeypatch: Any) -> None:
        """Test dataset listing with pagination and filtering"""
        mock_datasets = [
            DatasetInfo(id="dataset1", name="Dataset 1"),
            DatasetInfo(id="dataset2", name="Dataset 2")
        ]
        
        list_response = ListDatasetsResponse.builder().data(mock_datasets).total(2).page(1).limit("20").has_more(False).build()
    ...
    def test_dataset_retrieve_with_advanced_config(self, dataset_resource: Dataset, request_option: RequestOption, monkeypatch: Any) -> None:
        ...
        reranking_model = RerankingModel.builder().reranking_provider_name("cohere").reranking_model_name("rerank-english-v2.0").build()
        retrieval_model = RetrievalModel.builder().search_method("hybrid_search").reranking_enable(True).reranking_model(reranking_model).top_k(10).score_threshold_enabled(True).score_threshold(0.7).build()
        
        retrieve_response = RetrieveDatasetResponse(
            query=QueryInfo(content="advanced query"),
            records=[]
        )
        mock_execute = Mock(return_value=retrieve_response)
        monkeypatch.setattr("dify_oapi.core.http.transport.Transport.execute", mock_execute)
        
        retrieve_request = RetrieveDatasetRequest.builder().dataset_id("test-id").query("advanced query").retrieval_model(retrieval_model).build()
    ...
    def test_edge_cases(self, dataset_resource: Dataset, request_option: RequestOption, monkeypatch: Any) -> None:
        ...
        empty_list_response = ListDatasetsResponse.builder().data([]).total(0).page(1).limit("20").has_more(False).build()
