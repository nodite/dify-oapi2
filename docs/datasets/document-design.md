...
- **Resource Separation**: Each resource gets its own test file (e.g., `test_document_models.py`)
...
- **Public Class Separation**: Create separate files for public/common model tests (e.g., `test_document_public_models.py`)
- **Flat Structure**: All model test files are placed directly in `tests/knowledge_base/v1/model/` directory
- **Naming Convention**: Use `test_document_models.py` and `test_document_public_models.py` patterns

### Test Class Organization Pattern
**Within document test file, organize by API operations:**
```python
...
class TestCreateByFileModels:
    # Request tests (multipart/form-data)
    def test_request_builder(self): ...
    def test_request_file_handling(self): ...
    # RequestBody tests
    def test_request_body_builder(self): ...
    def test_request_body_data_serialization(self): ...
    # Response tests
    def test_response_inheritance(self): ...
...
```python
# test_document_public_models.py
